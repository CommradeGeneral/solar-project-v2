...
class ModbusClient extends EventEmitter {
    ...
    constructor({ host, port = 502, unitId = 1, timeout = 5000 } = {}) {
        ...
        this._socket = net.createConnection({ host, port, type: 'tcp' });
        this._client = new Modbus.client.TCP(this._socket, unitId, timeout);
...
// ── Usage example ──────────────────────────────────────────────────────────
const modbus = new ModbusClient({ host: '127.0.0.2', port: 502, timeout: 500 });

modbus.on('connect', () => console.log('connected'));
modbus.on('data', (data) => console.log('data =', data));
modbus.on('error', (err) => {
    console.log('error', err)

});

modbus.on('close', () => console.log('closed'));

setInterval(() => {
    modbus.readHoldingRegisters(1, 0, 1, 1).then((response) => {
